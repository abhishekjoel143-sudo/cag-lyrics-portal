"""Kannada PPT -> Unicode / Romanised lyric converter.

Extracted from kannada_ppt_lyrics_converter.py so the web app can reuse the
same conversion logic when the admin uploads a .pptx file.
"""
...
def convert_pptx_to_lyrics(pptx_path: Path) -> tuple[str, str]:
    """Return (kannada_text, english_pronunciation_text) with slides joined
    by blank lines, WITHOUT slide-number labels (per site requirement)."""
    kannada_parts = []
    english_parts = []
    for _slide_no, raw_lines in slide_texts(pptx_path):
        unicode_lines = [legacy_to_unicode(line) for line in raw_lines]
        if not unicode_lines:
            continue
        kannada_parts.append("\n".join(unicode_lines))
